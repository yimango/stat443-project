...
Table \ref{tab:return_models} summarizes the out-of-sample performance of AR, MA, and ARIMA models from the rolling-origin backtest.
...
\begin{itemize}
    \item \textbf{Selection Criterion}: Lowest out-of-sample RMSE (0.0100), indicating best overall forecast accuracy
    \item \textbf{Order Selection}: The order (2,0,2) was selected using in-sample BIC during backtesting, with average selected order across folds being approximately (2,0,2)
    \item \textbf{In-Sample Fit}: AIC: -16,287.1, BIC: -16,251.85 (from initial training fit, for diagnostic purposes)
    \item \textbf{Model Characteristics}: Combines benefits of both AR and MA components, better capturing complex return dynamics than pure AR or MA models
\end{itemize}

\subsubsection{Volatility Forecasting Models}

For volatility models (GARCH and ARCH), selection was based on information criteria (AIC/BIC) as these models are evaluated on their ability to capture volatility dynamics rather than point forecasts. Table \ref{tab:volatility_models} compares GARCH and ARCH models.
...
\begin{itemize}
    \item \textbf{Selection Criterion}: Lower BIC (-6.64 vs -6.14), indicating better model fit
    \item Lower AIC (-6.65 vs -6.14) confirms the selection
...
\begin{itemize}
    \item \textbf{ARIMA(2,0,2)} is selected based on lowest out-of-sample RMSE (0.0100)
    ...
    \item The use of out-of-sample RMSE for model type selection is justified because it directly measures forecast accuracy and avoids overfitting, which is appropriate for forecasting applications
\end{itemize}
